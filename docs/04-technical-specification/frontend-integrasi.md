...
## 6. Arsitektur Frontend (Frontend Architecture)

> **Status: belum ditulis.** Akan memuat: struktur design template (halaman, `assets/`, modul JS per-halaman), pemetaan halaman M-001 (`categories.html`, `items.html`, `pos.html`), serta pola `config.js` (`appCode`, `authBaseUrl`, `apiBaseUrl`).

## 7. Integrasi Frontend–Backend (Frontend–Backend Integration)

...
Modul Manajemen Produk & Kategori adalah fondasi data produk bagi seluruh alur transaksi POS. Modul ini mengelola katalog item yang dijual rumah makan: daftar menu, pengelompokan ke dalam kategori, penetapan harga, serta varian dan modifier (misalnya level pedas, pilihan topping, ukuran porsi). Data yang dikelola di sini menjadi sumber bagi modul Order (M-002), Pembayaran (M-005), dan Inventori (M-006), sehingga akurasi dan kerapian data menu sangat menentukan kelancaran operasional.
...
| **Tujuan Bisnis** | Menyediakan katalog menu yang akurat, terstruktur, dan mudah diperbarui sebagai dasar seluruh transaksi |
| **Modul Terkait** | M-002 (Order) menggunakan katalog; M-005 (Pembayaran) memakai harga; M-006 (Inventori) menautkan resep/BOM ke menu; M-007 (Promo) merujuk item & kategori |

**Cakupan modul (in scope):**

Pengelolaan kategori menu (buat, ubah, urutkan, aktif/nonaktif); pengelolaan item menu beserta harga, deskripsi, dan foto; pengelolaan varian (mis. ukuran) dan modifier (mis. topping, level pedas) beserta penyesuaian harganya; serta status ketersediaan menu (tersedia/habis).
...
|----|-------|------------|-----------|
| US-001 | Pemilik/Manajer | Sebagai pemilik, saya ingin membuat dan mengubah **kategori menu** (mis. Makanan, Minuman, Cemilan), agar daftar menu tersusun rapi dan mudah ditemukan kasir. | Must Have |
...
| US-003 | Pemilik/Manajer | Sebagai pemilik, saya ingin menambah, mengubah, dan menghapus **item menu** beserta nama, deskripsi, dan kategorinya, agar katalog selalu mencerminkan menu yang benar-benar dijual. | Must Have |
| US-004 | Pemilik/Manajer | Sebagai pemilik, saya ingin menetapkan **harga jual** untuk setiap item menu, agar kasir menagih harga yang konsisten dan benar. | Must Have |
| US-005 | Pemilik/Manajer | Sebagai pemilik, saya ingin menambahkan **foto** pada item menu, agar kasir/pelayan lebih cepat dan akurat memilih menu saat order. | Could Have |
...
| US-009 | Pemilik/Manajer | Sebagai pemilik, saya ingin menandai item menu sebagai **tersedia atau habis (sold out)**, agar kasir tidak menjual menu yang stoknya kosong. | Must Have |
...
| US-011 | Kasir/Pelayan | Sebagai kasir, saya ingin **mencari dan memfilter** menu berdasarkan nama atau kategori, agar saya bisa memilih item dengan cepat saat melayani pelanggan. | Must Have |
| US-012 | Kasir/Pelayan | Sebagai kasir, saya ingin melihat **hanya menu yang tersedia** beserta varian dan modifier-nya saat membuat order, agar tidak salah menjual menu yang habis atau nonaktif. | Must Have |
...
| FR-002 | Sistem harus memungkinkan pengguna **mengubah dan menghapus** kategori. Penghapusan ditolak jika masih ada item menu yang tertaut pada kategori tersebut (lihat Bagian 4). | US-001 | Must Have |
...
**B. Manajemen Item Menu**

| ID | Kebutuhan Fungsional | Sumber | Prioritas |
|----|----------------------|--------|-----------|
| FR-005 | Sistem harus memungkinkan pengguna **membuat item menu** dengan atribut: nama (wajib), kategori (wajib, satu kategori), deskripsi (opsional), dan harga jual (wajib). | US-003, US-004 | Must Have |
...
| FR-007 | Sistem harus memungkinkan pengguna **mengubah dan menghapus** item menu. Item yang masih direferensikan oleh transaksi tidak dapat dihapus; sistem menolak penghapusan tersebut (lihat Bagian 4). Penonaktifan item (`is_active`) adalah fungsi terpisah, bukan pengganti penghapusan. | US-003 | Must Have |
| FR-008 | Sistem harus memungkinkan pengguna **mengunggah satu foto** per item menu dengan batas format dan ukuran tertentu, serta menampilkannya pada layar order. | US-005 | Could Have |
| FR-009 | Sistem harus memungkinkan pengguna **mengaktifkan/menonaktifkan** item menu tanpa menghapus data historisnya. | US-010 | Should Have |
...
Business Rule adalah ketentuan bisnis yang harus selalu dipenuhi sistem, terlepas dari antarmuka atau alur yang dipakai. Aturan ini menjaga integritas data menu dan mencegah kondisi yang merusak transaksi.
...
| BR-002 | **Kategori tidak dapat dihapus** selama masih ada item menu yang tertaut padanya. Pengguna harus memindahkan atau menghapus item tersebut terlebih dahulu, atau cukup menonaktifkan kategori. | FR-002, FR-004 |
| BR-003 | **Setiap item menu wajib tertaut tepat pada satu kategori.** Item tidak boleh berada tanpa kategori. | FR-005 |
...
| BR-006 | **Item atau kategori berstatus nonaktif/habis tidak boleh muncul pada layar pembuatan order**, namun tetap tampil pada modul pengelolaan menu. | FR-004, FR-010, FR-016 |
...
| ID | Skenario (Given–When–Then) | Verifikasi FR |
|----|----------------------------|---------------|
| AC-005 | **Diberikan** form item menu, **ketika** pengguna menyimpan item dengan nama, kategori, dan harga valid, **maka** item tersimpan dan tampil di kategori yang dipilih. | FR-005 |
| AC-006 | **Diberikan** form item menu, **ketika** pengguna mengisi harga negatif atau non-numerik, **maka** sistem menolak simpan dan menampilkan pesan validasi harga. | FR-006, BR-004 |
...
| AC-008 | **Diberikan** sebuah item berstatus "Tersedia", **ketika** pengguna menandainya "Habis", **maka** item tidak lagi dapat dipilih pada layar pembuatan order namun tetap terlihat di pengelolaan menu. | FR-010, BR-006 |
...
| ID | Skenario (Given–When–Then) | Verifikasi FR |
|----|----------------------------|---------------|
| AC-013 | **Diberikan** daftar menu berisi item aktif dan nonaktif/habis, **ketika** kasir membuka layar order, **maka** hanya item aktif dan tersedia yang ditampilkan. | FR-016, BR-006 |
| AC-014 | **Diberikan** katalog menu, **ketika** kasir mengetik sebagian nama (mis. "ayam") tanpa memperhatikan huruf besar/kecil, **maka** sistem menampilkan seluruh item yang namanya mengandung kata tersebut. | FR-015 |
